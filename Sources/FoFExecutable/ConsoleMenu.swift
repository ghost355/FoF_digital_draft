...
mainLoop: while true {
    ...
    case "1":
        guard let ncm = getIntInput(prompt: "NCM (-4 to +6):", min: -4, max: 6, zeroValid: true) else { continue }
        if let result = engine.combatResult(ncm: ncm) {
    ...
    case "2":
        ...
        guard let expChoice = readLine(), let choice = Int(expChoice), choice >= 1, choice <= 3 else { continue }
        if choice == 0 { continue }
        let levels = ["veteran", "line", "green"]
        if let result = engine.hitEffect(experience: levels[choice - 1]) {
    ...
    case "3":
        guard let count = getIntInput(prompt: "Card count:", min: 1, max: 51, zeroValid: true) else { continue }
        guard let canJam = getBoolInput(prompt: "Can Jam?") else { continue }
        if let result = engine.grenadeAttack(count: count, canJam: canJam) {
    ...
    case "4":
        ...
        if let result = engine.callForFire(count: count, hasBnFireMission: bnFire) {
            let isSuccess = result != "short" && result != "failure"
            formatResult(result, isSuccess: isSuccess)
        }
        
    case "5":
        if let result = engine.mines() {
            formatResult(result, isSuccess: result == "success")
        }
        
    case "6":
        if let result = engine.hqEvent() {
            formatResult(result, isSuccess: result == "success")
        }
        
    case "7":
        guard let count = getIntInput(prompt: "Card count:", min: 1, max: 51, zeroValid: true) else { continue }
        if let result = engine.infiltration(count: count) {
            formatResult(result, isSuccess: result == "success")
        }
        
    case "8":
        guard let count = getIntInput(prompt: "Card count:", min: 1, max: 51, zeroValid: true) else { continue }
        if let result = engine.cover(count: count) {
            formatResult(result, isSuccess: result == "success")
        }
        
    case "9":
        guard let count = getIntInput(prompt: "Card count:", min: 1, max: 51, zeroValid: true) else { continue }
        if let result = engine.rally(count: count) {
            formatResult(result, isSuccess: result == "success")
        }
        
    case "a":
        guard let count = getIntInput(prompt: "Card count:", min: 1, max: 51, zeroValid: true) else { continue }
        if let result = engine.spotting(count: count) {
            formatResult(result, isSuccess: result == "success")
        }
        
    case "b":
        guard let count = getIntInput(prompt: "Card count:", min: 1, max: 51, zeroValid: true) else { continue }
        if let result = engine.contact(count: count) {
            formatResult(result, isSuccess: result == "success")
        }
    ...
    case "c":
        guard let count = getIntInput(prompt: "Card count:", min: 1, max: 51, zeroValid: true) else { continue }
        guard let canJam = getBoolInput(prompt: "Can Jam?") else { continue }
        if let result = engine.concentrateFire(count: count, canJam: canJam) {
            let isSuccess = result != "jam" && result != "failure"
            formatResult(result, isSuccess: isSuccess)
        }
