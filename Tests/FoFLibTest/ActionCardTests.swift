...
final class ActionCardTests: XCTestCase {
    ...
    private func makeCard(
        ...
        combatResults: [CombatResult] = [.miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss],
    ...
    func testCombatResult_ncmMinus4_returnsFirstElement() {
        let card = makeCard(combatResults: [.miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .hit])
        XCTAssertEqual(card.combatResult(for: -4), .miss)
    }
    
    func testCombatResult_ncm0_returnsFifthElement() {
        // ncm + 4 = 0 + 4 = index 4
        let card = makeCard(combatResults: [.miss, .miss, .miss, .miss, .hit, .miss, .miss, .miss, .miss, .miss, .miss])
        XCTAssertEqual(card.combatResult(for: 0), .hit)
    }
    
    func testCombatResult_ncm6_returnsLastElement() {
        let card = makeCard(combatResults: [.miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .hit])
        XCTAssertEqual(card.combatResult(for: 6), .hit)
    }
    
    func testCombatResult_ncmBelowMinus4_clampedToMinus4() {
        let card = makeCard(combatResults: [.miss, .hit, .hit, .hit, .hit, .hit, .hit, .hit, .hit, .hit, .hit])
        XCTAssertEqual(card.combatResult(for: -10), .miss)
    }
    
    func testCombatResult_ncmAbove6_clampedTo6() {
        let card = makeCard(combatResults: [.miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .miss, .hit])
        XCTAssertEqual(card.combatResult(for: 10), .hit)
    }
